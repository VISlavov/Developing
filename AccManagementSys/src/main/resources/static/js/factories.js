...
app.factory('RequestUtil', function($timeout, $q, $http, $filter, SettingsData, MessagesUtil, ScopeApplier){
	...
	factory.assembleFormSettings = function(scope, attr) {
		//format birthdate
		(function() {
			if(typeof scope.formData.birthDate != 'undefined') {
				scope.formData.birthDate = $filter('date')(scope.formData.birthDate, SettingsData.dateFormat);
			}
		})();
	...
	factory.getUser = function(scope, email) {
		$http.get('/getUser', {
			params: {
				email: email
			}
			...
			var data = response.data;
			ScopeApplier.applyPush(scope, "users", data);
		});
	};

	factory.getUsers = function(scope) {
		$http.get('/getUsers').success(function(data) {
			ScopeApplier.apply(scope, "users", data);
		});
	};
